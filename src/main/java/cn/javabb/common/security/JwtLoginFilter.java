...
public class JwtLoginFilter extends UsernamePasswordAuthenticationFilter {
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response,
        ...
        String access_token = JwtUtil.buildToken(user.getUsername(), ConsVal.TOKEN_EXPIRE_TIME, ConsVal.TOKEN_KEY);
        // 记录登录日志
        loginRecordService.saveAsync(user.getUsername(), LoginRecordDO.TYPE_LOGIN, null, request);
        // 返回json数据
    ...
    protected void unsuccessfulAuthentication(HttpServletRequest request, HttpServletResponse response,
        ...
        if (e instanceof UsernameNotFoundException) {
            ...
            loginRecordService.saveAsync(username, LoginRecordDO.TYPE_ERROR, "账号不存在", request);
        } else if (e instanceof BadCredentialsException) {
            result = JsonResult.error("账号或密码错误");
            loginRecordService.saveAsync(username, LoginRecordDO.TYPE_ERROR, "账号或密码错误", request);
        } else if (e instanceof LockedException) {
            result = JsonResult.error("账号被锁定");
            loginRecordService.saveAsync(username, LoginRecordDO.TYPE_ERROR, "账号被锁定", request);
        } else {
