...
public class UserController extends BaseController {
    ...
    public JsonResult list(HttpServletRequest request) {
        PageParam<UserDO> pageParam = new PageParam<>(request);
        List<UserDO> records = userService.listAll(pageParam.getNoPageParam());
    ...
    public JsonResult get(@PathVariable("id") Integer id) {
        PageParam<UserDO> pageParam = new PageParam<>();
    ...
    @ApiOperation("添加用户")
    @PostMapping()
    public JsonResult add(@RequestBody UserDO user) {
        user.setState(0);
    ...
    @ApiOperation("修改用户")
    @PutMapping()
    public JsonResult update(@RequestBody UserDO user) {
        user.setState(null);
        user.setPassword(userService.encodePsw(user.getPassword()));
    ...
    @ApiOperation("批量修改用户")
    @PutMapping("/batch")
    public JsonResult removeBatch(@RequestBody BatchParam<UserDO> batchParam) {
    ...
    public JsonResult updateStateBatch(@RequestBody BatchParam<UserDO> batchParam) {
        UserDO user = new UserDO();
    ...
    public JsonResult resetPswBatch(@RequestBody BatchParam<UserDO> batchParam) {
        UserDO user = new UserDO();
    ...
    public JsonResult importBatch(MultipartFile file) {
        ...
        try {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                user.setPhone(phone);
                user.setEmail(email);
                DictionaryDataDO sexDictData = dictionaryDataService.listByDictCodeAndName("sex", sexName);
                if (sexDictData == null) {
                    sb.append("第");
                    sb.append(i + startRow + 1);
                    sb.append("行第4");
                ...
                OrganizationDO org = organizationService.getOne(new QueryWrapper<OrganizationDO>().eq("organization_full_name", orgName), false);
                if (org == null) {
                    sb.append("第");
                    sb.append(i + startRow + 1);
                    sb.append("行第8");
