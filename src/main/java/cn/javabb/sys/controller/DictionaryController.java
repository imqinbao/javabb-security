...
import cn.javabb.sys.entity.DictionaryDO;
...
public class DictionaryController extends BaseController {
    ...
    @ApiPageParam
    @GetMapping("/page")
    public PageResult<DictionaryDO> page(HttpServletRequest request) {
        PageParam<DictionaryDO> pageParam = new PageParam<>(request);
    ...
    @ApiOperation("添加字典")
    @PostMapping()
    public JsonResult add(@RequestBody DictionaryDO dictionary) {
        if (dictionaryService.count(new QueryWrapper<DictionaryDO>().eq("dict_code", dictionary.getDictCode())) > 0) {
            return JsonResult.error("字典标识已存在");
        }
        if (dictionaryService.count(new QueryWrapper<DictionaryDO>().eq("dict_name", dictionary.getDictName())) > 0) {
    ...
    @ApiOperation("修改字典")
    @PutMapping()
    public JsonResult update(@RequestBody DictionaryDO dictionary) {
        if (dictionaryService.count(new QueryWrapper<DictionaryDO>().eq("dict_code", dictionary.getDictCode())
                .ne("dict_id", dictionary.getDictId())) > 0) {
            return JsonResult.error("字典标识已存在");
        }
        if (dictionaryService.count(new QueryWrapper<DictionaryDO>().eq("dict_name", dictionary.getDictName())
                .ne("dict_id", dictionary.getDictId())) > 0) {
            return JsonResult.error("字典名称已存在");
        }
    ...
    @ApiOperation("批量添加字典")
    @PostMapping("/batch")
    public JsonResult saveBatch(@RequestBody List<DictionaryDO> list) {
        // 对集合本身进行非空和重复校验
        ...
        // 数据库层面校验
        if (dictionaryService.count(new QueryWrapper<DictionaryDO>().in("dict_code",
                list.stream().map(DictionaryDO::getDictCode).collect(Collectors.toList()))) > 0) {
            return JsonResult.error("字典标识已存在");
        }
